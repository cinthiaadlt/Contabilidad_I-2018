...
$tipo = $_POST["tipo_hdn"];
/* Variables del formulario de creación de clase */

$num_clase 			= $_POST["num_clase"];
...
switch ($tipo) {
	case '1':
		$sql = "SELECT * FROM clasificaciones WHERE codigo_clasificacion='$codigo_clasificacion'";
		$ejecutar_consulta = $conexion->query($sql);
		if($ejecutar_consulta->num_rows > 0){
			$mensaje = "Ya existe una clase con el código <strong>".$codigo_clase."</strong>";
			header("Location: alta-cuentas.php?error=si&mensaje=$mensaje");
		} else {
			$sql = "INSERT INTO `sic115`.`clasificaciones` (`codigo_clasificacion`, `nombre_clasificacion`) VALUES ('$num_clase', '$nombre_clase')";
			$ejecutar_consulta = $conexion->query(utf8_decode($sql));
			if($ejecutar_consulta){
				$mensaje = "Se ha dado de alta la clase: <strong>".$codigo_clase."</strong>.";
